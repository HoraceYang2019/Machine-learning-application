...
logreg  = LogisticRegression(max_iter = 10000, penalty='l1', solver='liblinear').fit(X_train_7, y_train)
rfreg  = RandomForestRegressor().fit(X_train_7, y_train)

print(f"Training set score of LR: {logreg.score(X_train_7, y_train):.3f}")
print(f"Training set score of RFR: {rfreg.score(X_train_7, y_train):.3f}")
#%%
import shap
